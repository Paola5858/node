  ...
  return <section className="cadastro-page auth-page">
    ...
    <div className="form-layout"><div className="form-card glass-card"><div className="card-heading"><div><span className="mini-label">dados da conta</span><h2>vamos deixar isso pessoal.</h2></div><span className="required-note">* campos obrigatórios</span></div>
      ...
      <form onSubmit={salvar} noValidate><div className="form-grid">
        <div className={`field ${erros.nome ? 'has-error' : ''}`}><label htmlFor="nome">nome<span>*</span></label><input id="nome" name="nome" value={valores.nome} onChange={atualizar} placeholder="ex.: Paola Soares" />{erros.nome && <small className="field-error">{erros.nome}</small>}</div>
        <div className={`field ${erros.email ? 'has-error' : ''}`}><label htmlFor="email">e-mail<span>*</span></label><input id="email" name="email" type="email" value={valores.email} onChange={atualizar} placeholder="voce@exemplo.com" />{erros.email && <small className="field-error">{erros.email}</small>}</div>
        <div className={`field ${erros.senha ? 'has-error' : ''}`}><label htmlFor="senha">senha<span>*</span></label><input id="senha" name="senha" type="password" value={valores.senha} onChange={atualizar} placeholder="mínimo de 6 caracteres" />{erros.senha && <small className="field-error">{erros.senha}</small>}</div>
        <div className={`field ${erros.confirmarSenha ? 'has-error' : ''}`}><label htmlFor="confirmarSenha">confirmar senha<span>*</span></label><input id="confirmarSenha" name="confirmarSenha" type="password" value={valores.confirmarSenha} onChange={atualizar} placeholder="repita sua senha" />{erros.confirmarSenha && <small className="field-error">{erros.confirmarSenha}</small>}</div>
